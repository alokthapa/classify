...
(defvar *totals* (make-hash-table :test #'equal))
(defvar *types* '())

(defparameter *max-chars* (* 10 1024))
(defparameter *corpus* (make-array 1000 :adjustable t :fill-pointer 0))

(defun print-features (text)
  (mapcar #'(lambda(word)(format t "for word ~a ~%" word)
		   (if (gethash word *feature-database*)
		       (with-slots (cat-count )(gethash word *feature-database*)
			 (maphash #'(lambda (k v)
				      (format t "~a with val ~a~%~%" k v)) cat-count))) (extract-words text))))
...
(defun create-types (dirname)
	     (mapcar #'(lambda(dir) (let ((type (last1 (pathname-directory (pathname  dir)))))
				      (push type *types*)
				      (setf (gethash type *totals*) 0))) (list-directory dirname)))

(defun classify (text)
  "Classify the text of a message as SPAM, HAM, or UNSURE."
  (classify-features (extract-features text)))


(defun classify-file (file)
  (classify (get-text file)))
...
(defmethod initialize-instance :after ((feature word-feature) &key)
  (with-slots (cat-count) feature
    (mapcar #'(lambda (type)
		(setf (gethash type cat-count) 0))
	    *types*)))
...
(defun total-count (feature)
  (with-slots (cat-count) feature
    (let ((count 0))
      (maphash #'(lambda (k v) (+ count v)) cat-count)
      count)))

(defun type-probability (feature type)
  (with-slots (cat-count) feature
    (/ (1+ (gethash type cat-count)) (+ (length *types*) (total-count feature)))))
  
(defun bayesian-probability(features type &optional (doc-probability 1/20))
  (+ (log doc-probability)
     (reduce #'+
	     (mapcar #'(lambda (f)
...
(defun classify-features (features)
  (let ((prob-vals '()))
    (setf prob-vals 
	  (mapcar #'(lambda (type)
		      (bayesian-probability features type)) 
		  *types*))
	  (format t "~a" prob-vals)
	  (format t "~a" *types*)
...
(defun train-from-dataset (dirname datasize)
	   (mapcar #'(lambda (dir)
		       (format t "In directory ~a~%" dir)
		       (mapcar #'(lambda (file)
				   (format t "Processing file ~a~%" file)
				   (train (get-text file) (last1 (pathname-directory (pathname  dir)))))
			       (nthcdr (- 1000 datasize) (list-directory dir))))
